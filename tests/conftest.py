...
active-transcript store (``winnow-active-sessions.json``) that
...
def _disable_real_receipts(monkeypatch):
    """Never write prune receipts to the developer's real ``~/.winnow`` from a
    ...
    ~/.winnow. (Receipt-writer tests stay safe under bare unittest — they
