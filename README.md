...
| — the 100-fork resume test | **not validated; attempted 2026-09-07 and blocked by the environment**, not by the corpus ([docs/MILESTONE-2-RESULTS.md](docs/MILESTONE-2-RESULTS.md) §1). The dry run walked all 1,087 sessions of `~/.claude/projects`: **212 were forkable against a target of 100**, and `cold-age` refused 5 of them, so the population is there. No fork could be written and no `claude` child could reach a model, so nothing was resumed. Still needs an environment that can do both |
